        ...
        <>
            {
                arr.length > 1 && arr.map((p, i) => {
                    return <span key={p} className={"border border-gray-300 py-2 px-4 mx-2 sm:mx-4 my-2 cursor-pointer" + (Number(activePage) === i + 1 ? " bg-black text-white" : "")} data-id={i + 1} onClick={(e) => props.handleClick(e)}>{p}</span>
